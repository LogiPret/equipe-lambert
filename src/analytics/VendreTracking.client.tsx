...
// and button clicks within the vendre landing page.
export default function VendreTracking() {
  const pathname = usePathname()

  useEffect(() => {
    if (pathname !== '/vendre') return
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && entry.intersectionRatio >= 0.5) {
            ...
            if (!seen.has(id)) {
              seen.add(id)
              track('block_view', { page: 'vendre', block_type: type, block_index: index })
            }
          }
        })
      },
